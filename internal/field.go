...
readSecretMap:
	const convertErrFormat = "failed to convert secret \"%s's\" key, \"%s\" to %s: %w"
	...
	if value, ok := secretMap[f.MapKeyName]; ok {
		...
		case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
			...
			if err != nil {
				...
				return fmt.Errorf(convertErrFormat, f.SecretName, f.MapKeyName, t, err)
			}
		...
		case reflect.Bool:
			...
			if err != nil {
				return fmt.Errorf(convertErrFormat, f.SecretName, f.MapKeyName, "bool", err)
			}
			f.Value.SetBool(v)
